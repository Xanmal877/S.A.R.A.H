"""Shared utilities for S.A.R.A.H. Discord cogs: absolute paths and atomic JSON persistence."""
...
MOD_CONFIG_PATH = DATA_DIR / "mod-config.json"
WARNINGS_PATH = DATA_DIR / "warnings.json"
GUILD_CONFIG_PATH = DATA_DIR / "guild-config.json"
...
def load_json_list(path) -> list:
    """Load a JSON array from *path*, returning [] for missing files.

    Corrupt or non-array files are backed up with a .bak suffix instead of
    being silently discarded, so live data is never lost.
    """
    path = Path(path)
    if not path.exists():
    ...
    if not isinstance(data, list):
        logger.warning("JSON file %s is not an array; treating as empty", path)
        return []
    return data
